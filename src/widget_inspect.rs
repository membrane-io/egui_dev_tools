...
impl Plugin for WidgetInspect {
  ...
  fn input_hook(&mut self, input: &mut RawInput) {
    for event in input.events.iter() {
      match event {
        Event::Key { key: Key::I, repeat: false, pressed: true, modifiers, .. } if modifiers.command => {
        ...
        match e {
          ...
          Event::MouseWheel { delta, unit, .. } => {
            if *unit == MouseWheelUnit::Line || *unit == MouseWheelUnit::Page {
              self.selected_widget = self.selected_widget.saturating_add_signed(delta.y.signum() as isize);
            } else {
          ...
          Event::Key { key: Key::Tab, repeat: false, pressed: true, .. } => {
...
fn paint_info(
  ...
  let strong = TextFormat { font_id: font.clone(), color: text_color, valign: Align::Center, ..Default::default() };
  let weak = TextFormat {
    font_id: font.clone(),
  ...
  let unselected_marker_format = TextFormat { color: Color32::DARK_GRAY, ..selected_marker_format.clone() };
  ...
  let mut header_job = LayoutJob::default();
  {
    ...
    header_job.append(
      "EGUI",
      0.0,
      TextFormat { underline: config.show_egui_frames.then(|| stroke).unwrap_or_default(), ..strong_small.clone() },
    );
    header_job.append(" ", 0.0, weak_small.clone());
    header_job.append(
      "STD",
      0.0,
      TextFormat { underline: config.show_std_frames.then(|| stroke).unwrap_or_default(), ..strong_small.clone() },
    );
    header_job.append(" ", 0.0, weak_small.clone());
    header_job.append(
      "ALL",
  ...
  let left_side = |frame: &ParsedFrame| match frame {
    ParsedFrame::Parsed(location) => {
      ...
      let function = if location.symbol.function().len() > max_function_len {
        location.symbol.function().chars().take(max_function_len - 1).collect::<String>() + &ELLIPSIS
      } else {
        location.symbol.function().to_string()
      };
      (format!("{}{}::{}", indent, location.symbol.type_(), function,), format)
    }
    ParsedFrame::Failed(text) => (format!("! {}", text.chars().take(800).collect::<String>()), weak.clone()),
  };
  ...
  let columns = callstack.iter().map(|frame| (left_side(frame), right_side(frame)));
  ...
  for (i, ((left, left_format), (right, right_format))) in columns.into_iter().enumerate() {
    ...
    if i == most_significant_frame {
      ...
      left_job.append(UNSELECTED_MARKER, space_width, unselected_marker_format.clone());
    } else {
      left_job.append(
        UNSELECTED_MARKER,
        space_width,
        TextFormat { color: Color32::TRANSPARENT, ..selected_marker_format.clone() },
      );
    };
  ...
  if row_count == 0 {
    ...
    left_job.append("Consider setting up `wasm-stack-trace` to get proper Rust symbols.\n\n", 0.0, weak_small.clone());
    left_job.append("  More info: ", 0.0, weak_small.clone());
    left_job.append("https://github.com/membrane-io/wasm-stack-trace\n", 0.0, strong_small.clone());
  }
  ...
  let total_size = vec2(header_size.x.max(body_size.x), header_size.y + body_size.y + GAP);
  ...
  if bounds.bottom() > screen_rect.bottom() {
    bounds = bounds.translate(-vec2(0.0, (bounds.bottom() - screen_rect.bottom()).min(bounds.top())));
  }
  ...
  // Paint text
  painter.galley(left_rect.left_top() + Vec2::splat(MARGIN), left_galley, text_color);
  ...
  while i > 0 {
    ...
    if big_enough(&overlap) {
      ...
      if overlap.min.y > rect.min.y {
        let lt = Rect { min: rect.min, max: overlap.right_top() };
      ...
      if overlap.max.x < rect.max.x {
        let rt = Rect { min: Pos2 { x: overlap.max.x, y: rect.min.y }, max: Pos2 { x: rect.max.x, y: overlap.max.y } };
        if big_enough(&rt) {
          rects.push(rt);
        }
      }
      ...
      if overlap.max.y < rect.max.y {
        let rb = Rect { min: overlap.left_bottom(), max: rect.max };
        if big_enough(&rb) {
          rects.push(rb);
        }
      }
      ...
      if overlap.min.x > rect.min.x {
        let lb = Rect { min: Pos2 { x: rect.min.x, y: overlap.min.y }, max: Pos2 { x: overlap.min.x, y: rect.max.y } };
        if big_enough(&lb) {
          rects.push(lb);
        }
      }
      ...
      backtrace::resolve_frame(frame, |resolved| {
        let Some(path) = resolved.filename().map(|path| path.to_string_lossy()) else {
          parsed_frames.push(ParsedFrame::Failed(
            resolved.name().map_or_else(|| "<unknown>".to_string(), |name| name.to_string()),
          ));
          return;
        };
...
impl Callstack {
  ...
  fn raw(&self) -> Option<String> {
    js_sys::Reflect::get(&self.0, &wasm_bindgen::JsValue::from_str("stack")).ok().and_then(|stack| stack.as_string())
  }
