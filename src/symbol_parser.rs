...
impl Symbol {
  ...
  pub fn parse(input: &str) -> Self {
    match SymbolExtractor::new(input).extract() {
      Ok(sym) => sym,
      Err(err) => {
        Symbol { function: input.to_string(), type_: format!("(err: {})", err), crate_: "<unknown-crate>".to_string() }
      }
    }
  }
...
impl<'a> SymbolExtractor<'a> {
  ...
  fn extract(mut self) -> Result<Symbol, ParseError> {
    ...
    if self.pos < self.input.len() {
      return Err(self.error(format!("unexpected character '{}' at end of input", self.current_char().unwrap_or('?'))));
    }
  ...
  fn error(&self, message: String) -> ParseError {
    ParseError { message, position: self.pos }
  }
  ...
  fn parse_simple_path(&mut self) -> Result<SimplePathInfo<'a>, ParseError> {
    ...
    Ok(SimplePathInfo { crate_: first, type_for_simple: second_to_last, innermost: last, function_with_closures })
  }
  ...
  fn skip_qualified_path(&mut self) -> Result<Option<&'a str>, ParseError> {
    ...
    while self.peek_str("::") {
      ...
      if self.current_char().is_none() || self.peek_char() == Some('>') || self.peek_char() == Some(',') {
        break;
      }
    ...
    if !has_segments {
      ...
      match base {
        TypeInfo::SimplePath(info) => {
          let type_ = if for_type { info.innermost } else { info.type_for_simple };
...
enum TypeInfo<'a> {
  Empty,
  ImplTarget { crate_: Option<&'a str>, type_: Option<&'a str> },
